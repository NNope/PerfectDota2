...
@interface  MJRefreshBaseView()
{
    ...
    __weak UIImageView *_refreshingView;
    BOOL _endingRefresh;
}
@end
...
{
    if (!_arrowImage) {
        UIImageView *arrowImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:MJRefreshSrcName(@"loading_arrow_down")]];
...
/**
  *  修改刷新图片 修改控件
  */
- (UIImageView *)refreshingView
{
    if (!_refreshingView) {
        UIImageView *refreshingImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:MJRefreshSrcName(@"loading")]];
        refreshingImage.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
        [self addSubview:_refreshingView = refreshingImage];
    }
...
- (void)startRefresh
{
    ...
    [self.refreshingView.layer addAnimation:anima forKey:@"rotateAnimation"];
}
- (void)stopRefresh
{
...
- (void)layoutSubviews
{
    ...
    self.arrowImage.center = CGPointMake(arrowX, self.mj_height * 0.47);
    
    // 2.指示器
//    self.activityView.center = self.arrowImage.center;
    
    // 3.修改的刷新指示器
    self.refreshingView.center = self.arrowImage.center;
}
...
//                    self.activityView.alpha = 0.0;// 修改了MJ的菊花 不要了
                    self.refreshingView.alpha = 0.0;// 新增
                } completion:^(BOOL finished) {
                    // 停止转圈圈
//                    [self.activityView stopAnimating];// 修改
                    [self stopRefresh];// 新增
                    // 恢复alpha
//                    self.activityView.alpha = 1.0;// 修改
                    self.refreshingView.alpha = 0;// 新增
                }];
...
//                [self.activityView stopAnimating];// 修改
        ...
        {
            // 开始转圈圈
//			[self.activityView startAnimating];// 修改
            self.refreshingView.alpha = 1.0;// 新增
            [self startRefresh];
            // 隐藏箭头
